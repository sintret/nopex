...
//accept for cron
router.post('/signal', async function (req,res) {
    var json = {}
...
router.get('/player/:uid', async function (req, res) {
    ...
    if(unique.hasOwnProperty(uid)){
        ...
        res.render('layouts/playease', {
            uid:uid,
            json:JSON.stringify(unique[uid]),
            renderBody: '/index/player.ejs',
            renderEnd: "/index/playerjs.ejs"
        })
...
router.get('/ts', async function (req,res) {
    res.render('layouts/main', {
        ...
        renderBody: '/index/ts.ejs',
        renderEnd: "/index/tsjs.ejs"
    })
});
